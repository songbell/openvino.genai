...
SequenceGroupSamplingInfo Sampler::sample_from_sequence_group(SequenceGroup::Ptr sequence_group, ov::Tensor sequence_group_logits, 
    ...
    if (sampling_params.is_greedy_decoding() || sampling_params.is_multinomial()) {
        ...
        for (size_t running_sequence_id = 0; running_sequence_id < num_running_sequences; ++running_sequence_id) {
            ...
            std::vector<int64_t> draft_generated_token_ids;
            ...
            for (size_t i = 0; i <= num_tokens_to_process; ++i) {
                ...
                if (max_num_sampled_token == 0 && is_validation_mode_enabled) {
                ...
                bool is_extend_sequence = (is_validation_mode_enabled && logit_token_offset == 0) || is_generate_n_tokens || !is_validation_passed;
                ...
                if (is_validation_mode_enabled) {
                    ...
                    // save for later
                    draft_generated_token_ids.push_back(sampled_token.m_index);
                }
            ...
            if (!is_validation_mode_enabled) {
                // remove the last 15 tokens
                auto removed_token_cnt = 15;
                auto generated_len = running_sequence->get_generated_len();
                auto generated_token_ids = running_sequence->get_generated_ids();
                for (size_t i = generated_len - removed_token_cnt; i < generated_len; ++i) {
                    logit_processor.decrease_generated_token_occurance(generated_token_ids[i]);
                }
                running_sequence->remove_last_tokens(removed_token_cnt);
                // register newly generated tokens
                // disard the first one
                draft_generated_token_ids.erase(draft_generated_token_ids.begin());
                for (auto& token_id : draft_generated_token_ids) {
                    logit_processor.register_new_generated_token(token_id);
                    running_sequence->append_token(token_id, 0.f); // log prob is not important for draft model generated tokens, since they are not validated
                }
            }
