...
def main():
    ...
    print()
    if (res.perf_metrics):
        print(f"Total generate time: {res.perf_metrics.get_generate_duration().mean:.2f} ms" )
        print(f"TTFT: {res.perf_metrics.get_ttft().mean:.2f} ± {res.perf_metrics.get_ttft().std:.2f} ms" )
        print(f"TPOT: {res.perf_metrics.get_tpot().mean:.2f} ± {res.perf_metrics.get_tpot().std:.2f} ms/iteration")
        print(f"Num generated token: {res.perf_metrics.get_num_generated_tokens()} tokens")
        print(f"Total iteration number: {len(res.perf_metrics.raw_metrics.m_new_token_times)}")
